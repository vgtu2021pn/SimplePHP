...
class BrowserDetect {
    ...
    const BROWSER_CURL = 'Curl';                         	  // Curl bot
    const BROWSER_YANDEX = 'Yandex'; 						  // Yandex, russian search engine
	...
	/**
	 * Determine if the browser is the Curl request or not
	 * @return boolean True if the browser is the Curl bot
	 */
	protected function checkCurlBot() {
		$retval = false;
		if( preg_match('/curl/i',$this->_agent) ) {
	...
	/**
	 * Determine if the browser is the Yandex bot (Russian search engine) request or not
	 * @return boolean True if the browser is the Curl bot
	 */
	protected function checkYandexBot() {
		$retval = false;
		if( preg_match('/Yandex/i',$this->_agent) ) {
			$aresult = explode('/',stristr($this->_agent,'yandex'));
			$aversion = explode(' ',$aresult[1]);
			$this->setVersion(str_replace(';','',$aversion[0]));
	...
	protected function checkPlatform() {
		if( preg_match('/iPhone/i', $this->_agent) ) {
			...
			$this->_os = self::OPERATING_SYSTEM_IPHONE_OS;
		}
		...
		elseif( preg_match('/mac/i', $this->_agent) ) {
			...
			$this->_os = self::OPERATING_SYSTEM_MAC_OS;
		}
		...
		if($this->_os == '') $this->_os = $this->_platform;
